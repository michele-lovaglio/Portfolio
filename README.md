# Portfolio
Il mio portfolio personale - HTML statico
# 👋 Ciao, sono Michele Lovaglio

🎓 Studente all’Istituto Tecnico Industriale (indirizzo informatico)  
💻 Appassionato di programmazione, reti e web design  
🚀 Attualmente sto seguendo:
- Cisco IT Essentials
- Responsive Web Design (FreeCodeCamp)

📚 Conoscenze:
- Linguaggi: C, Java, HTML, CSS, Assembly, VBA
- Pacchetto Office (Word, Excel, PowerPoint)

🎮 Certificazioni:
- Patentino droni A1/A3 e A2
- Inglese B2 – Trinity College & EF SET

🌍 Vivo ad Andria (BT), Italia  
📫 Contatti:
- Email: michele.lovaglio.dev@gmail.com
- Portfolio: [michele-lovaglio.github.io/portfolio](https://michele-lovaglio.github.io/portfolio)
